<template>
	<div class="hello">
		...
		<p>
			...
			check out the <a href="https://cli.vuejs.org" target="_blank"
		...
		<h3>Security</h3>
		<ul>
			<li><button @click="securetest">Secured</button></li>
			<li><button @click="opentest">Open</button></li>
			<p>Result: {{ secureTestResult }}</p>
		</ul>
	</div>
</template>
...
export default {
  ...
  props: {
	...
	methods: {
		securetest: function() {
			let state = this;
			this.rest.get('/securetest/secured').then(function (response) {
				console.log(response);
				state.secureTestResult = response.data;
			}).catch(function (error) {
				console.log(error);
				state.secureTestResult = error;
			});
		},
		opentest: function() {
			let state = this;
			this.rest.get('/securetest/open').then(function (response) {
				console.log(response);
				state.secureTestResult = response.data;
			}).catch(function (error) {
				console.log(error);
